...
function ModalGroup({ children, mode }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === "CREAR") {
      axios
        .post('http://10.10.200.12:8008/Rennueva/create-django-group/', nuevoDato)
        .then(response => {
    ...
    if (mode === "EDITAR") {
      axios
        .put('http://10.10.200.12:8008/Rennueva/update-django-group/', editarDato)
        .then(response => {
          const data = response.data;
          console.log(data)
          e.target.reset();
    ...
    if (mode === "BORRAR") {
      axios
        .put('http://10.10.200.12:8008/Rennueva/delete-django-group/', borrarDato)
        .then(response => {
          const data = response.data;
          console.log(data)
          e.target.reset();
