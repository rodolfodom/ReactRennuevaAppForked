...
function ModalVehicle({ children, mode }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === "CREAR") {
      axios
        .post('http://10.10.200.12:8008/Rennueva/create-vehicle/', nuevoDato)
        .then(response => {
    ...
    if (mode === "EDITAR") {
      axios
        .put('http://10.10.200.12:8008/Rennueva/update-vehicle/', editarDato)
        .then(response => {
          const data = response.data;
          console.log(data)
          setOpenModalText(true);
    ...
    if (mode === "BORRAR") {
      axios
        .post('http://10.10.200.12:8008/Rennueva/delete-vehicle/', { placas: oldVehicle})
        .then(response => {
          const data = response.data;
          console.log(data)
          setOpenModalText(true);
  ...
  useEffect(() => {
    ...
    const fetchVehicles =axios.get('http://10.10.200.12:8008/Rennueva/get-all-vehicle/')

    const fetchDrivers =axios.get('http://10.10.200.12:8008/Rennueva/get-all-drivers/')
