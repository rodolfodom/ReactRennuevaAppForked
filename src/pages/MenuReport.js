...
const getAllInfoReport = async (id_report) => {
  try {
    ...
    const response = await axios.post('http://10.10.200.12:8008/Rennueva/get-all-info-per-report/', {
      reportId: id_report,
    });
...
const savePdf = async(pdfBase64, id_report) => {
  ...
  try {
    // Usamos 'await' para esperar a que la solicitud se complete y para obtener la respuesta
    const response = await axios.post('http://10.10.200.12:8008/Rennueva/finish-report/', {
      reportId: id_report,
...
function MenuReport() {
  ...
  useEffect(() => {
    axios
      .get('http://10.10.200.12:8008/Rennueva/get-all-reports/')
      .then(response => {
