...
const getAllInfoReport = async (id_report) => {
  try {
    ...
    const response = await axios.post('https://api.rennueva.com/Rennueva/get-all-info-per-report/', {
      reportId: id_report,
    });
...
const savePdf = async(pdfBase64, id_report) => {
  ...
  try {
    // Usamos 'await' para esperar a que la solicitud se complete y para obtener la respuesta
    const response = await axios.post('https://api.rennueva.com/Rennueva/finish-report/', {
      reportId: id_report,
